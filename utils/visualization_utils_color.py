...
def visualize_boxes_and_labels_on_image_array(image,
    ...
    for box, color in box_to_color_map.items():
        ...
        # 将框放大到1.6倍
        xmin,ymin,xmax,ymax = xmin-0.3*(xmax-xmin), ymin-0.3*(ymax-ymin), xmax+0.3*(xmax-xmin), ymax+0.3*(ymax-ymin)
        if instance_masks is not None:
...
def write_boxes_to_file(filepath,
                          boxes,
                          classes,
                          scores,
                          min_score_thresh=.7
    ...
    for box, color in box_to_color_map.items():
        ...
        file.writelines("{:f}, {:f}, {:f}, {:f}".format(max(xmin,0), max(ymin,0), min(xmax,1), min(ymax,1)))
